...
`;
      ...
      document.addEventListener("click", async (event) => {
        ...
        if (otpButton) {
          ...
          if (vaultData2?.secrets?.O) {
            ...
            const secretDecoded = Bytes.base32.decode(vaultData2.secrets.O);
    ...
    async handleKeyDown(event) {
      if (event.ctrlKey && event.key === "\xF9") {
    ...
    renderVaultEntry(vault) {
      ...
      div.innerHTML = `<div class="vve-info"><strong>${vault.secrets.T ?? "No title"}</strong><span>${vault.secrets.U ?? "no username"}</span></div>
        ${!!vault.secrets.O ? '<button class="vve-totp" title="TOTP code">TOTP</button>' : ""}`;
